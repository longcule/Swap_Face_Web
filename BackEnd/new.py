...
def generateData(link_full1, link_full2):
    ...
    sukien_lists = [
        ...
        ['skGapNhau', 'skhanhphuc', 'skmuasam', 'skkethon', 'skngoaitinh', 'skToTinh'],
        ['skchiatay', 'skToTinh', 'skGapNhau', 'skhanhphuc', 'skkethon', 'skngoaitinh'],
        ['skGapNhau', 'skhanhphuc', 'skToTinh', 'skchaunoi', 'sklyhon', 'skmuasam']
    ]
...
def addThemSuKienTinhYeu(id_toan_bo_su_kien):
    ...
    # noidung_su_kien = "zbc"
    # so_thu_tu_su_kien = 12
    device_them_su_kien =  "23"
    ip_them_su_kien =   "sds"
    # link_da_swap =  "sdfsd"
    thoigian_sukien = "sdfsdfds"
    # ten_su_kien = "sdsfefsef"
    try:
        if connection.is_connected():
            print("Connected to MySQL database")
            cursor = connection.cursor()
        ...
        print("halllo", id_saved_max, date)
        sql = f"INSERT INTO saved_sukien (id_saved ,link_nam_goc , link_nu_goc ,link_nam_chua_swap , link_nu_chua_swap, link_da_swap , thoigian_swap , ten_su_kien , noidung_su_kien , id_toan_bo_su_kien ,so_thu_tu_su_kien, thoigian_sukien, device_them_su_kien, ip_them_su_kien) VALUES ( {id_saved_max}  ,%s  , %s  ,%s, %s, %s, %s, %s, %s, { id_toan_bo_su_kien },{so_thu_tu_su_kien}, %s, %s, %s)"
        val = (link_full1, link_full2, link_full1, link_full2, link_da_swap, date, ten_su_kien, noidung_su_kien, thoigian_sukien, device_them_su_kien, ip_them_su_kien)
        mycursor.execute(sql, val)
        result1 = mycursor.fetchall()
        ketqua = "id_saved ,link_nam_goc , link_nu_goc ,link_nam_chua_swap , link_nu_chua_swap, link_da_swap , thoigian_swap , ten_su_kien , noidung_su_kien , id_toan_bo_su_kien ,so_thu_tu_su_kien, thoigian_sukien, device_them_su_kien, ip_them_su_kien VALUES " + str(id_saved_max) + " " + str(link_full1) +" " +str(link_full2) + " " + str(link_full1) +" " +str(link_full2) + " " + str(link_da_swap)  + " " +str(date)  + " " +str(ten_su_kien) + " " + str(noidung_su_kien) +  " "  + str( id_toan_bo_su_kien) + " " + str(so_thu_tu_su_kien) +  " " + str(thoigian_sukien) +  " " + str( device_them_su_kien) +  " " + str(ip_them_su_kien)
